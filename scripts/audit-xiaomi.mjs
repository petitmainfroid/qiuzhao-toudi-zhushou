...
const snapshotPath = resolve(process.cwd(), "tests/fixtures/xiaomi-internship-schema.json");
...
assert.equal(response.ok, true, `Xiaomi page returned HTTP ${response.status}`);
const html = await response.text();
const rawWebsiteInfo = html.match(
  /<script id="js-websiteInfo" type="text\/json">([\s\S]*?)<\/script>/
)?.[1];
assert.ok(rawWebsiteInfo, "js-websiteInfo was not present in the Xiaomi page HTML");
...
const jobResponse = await fetch(
  `https://xiaomi.jobs.f.mioffice.cn/api/v1/job/posts/${expected.job.id}`,
  { headers: { accept: "application/json" } }
);
assert.equal(jobResponse.ok, true, `Xiaomi job API returned HTTP ${jobResponse.status}`);
const job = (await jobResponse.json()).data?.job_post_detail;
assert.equal(job?.title, expected.job.title);
assert.equal(job?.code, expected.job.code);

const fieldCount = groups.reduce((total, group) => total + group.children.length, 0);
console.log(
  `Verified Xiaomi ${expected.job.code} schema: ${groups.length} groups, ${fieldCount} visible fields, no application submission.`
);
