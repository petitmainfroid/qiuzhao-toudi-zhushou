...
- 保存基本信息（可选证件类型与号码）、教育、实习、工作、项目、作品、获奖、语言、求职偏好和常用回答。
...
- 内置一个不依赖 OpenCLI、Node.js 或本地服务的浏览器会话内核；用户点击“连接当前招聘页”后，它会固定该 HTTPS 标签页 10 分钟，并在跨站、到期、关闭或调试器冲突时停止。连接后可只读识别普通页面、同源 iframe 和开放 Shadow DOM 中的控件，并按标签、角色、名称、占位提示、附近文字和选项查找；结果只含语义摘要与会话内不透明引用。
- 记住同一网站中用户确认的字段纠错映射。
- 导出、导入或永久删除本地档案与映射；JSON 会包含证件号码等档案原文，PDF 字节不进入 JSON 导出，可单独删除或随全部本地数据清除。
...
- 不存储招聘网站密码、Cookie、验证码、身份验证令牌或浏览器会话。
- 可在明确告知尚未加密后，将证件类型与号码保存到本机档案；编辑器和预览遮罩号码，匹配后必须由用户逐项确认才会填写。
- 不填写密码、验证码、CAPTCHA、银行卡和隐藏字段。
- 不提供任意文件上传；只保存一份用户亲选的 PDF。每次附加前必须核对文件摘要、HTTPS 站点和简历控件，并只允许一次、60 秒内有效的尝试。其他附件仍由用户手动处理。
- 不绕过登录、短信验证、身份检查或网站风控。
- 不点击最终申请提交按钮。
- 证件资料、出生日期、性别、籍贯、政治面貌等敏感字段默认要求确认。
...
src/ats          ATS 家族检测、模板、证据与默认运行时组合
...
ATS 分层设计与当前证据边界见 [`docs/generic-kernel-feishu-adapter.md`](docs/generic-kernel-feishu-adapter.md)。
