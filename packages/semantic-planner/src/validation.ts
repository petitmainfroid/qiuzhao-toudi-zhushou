...
const PROFILE_PATH_PATTERN = /^[A-Za-z][A-Za-z0-9_]*(?:\.[A-Za-z][A-Za-z0-9_]*)*$/;
...
export function createAiPlannerRequest(value: unknown): Readonly<AiPlannerRequest> {
  if (!isRecord(value) || !exactKeys(value, ["fields", "profilePathCatalog"]) || !Array.isArray(value.fields)
...
export function parseAiPlannerRequest(value: unknown): Readonly<AiPlannerRequest> {
  if (!isRecord(value) || !exactKeys(value, ["schemaVersion", "runFlags", "fields", "profilePathCatalog"])
  ...
  return createAiPlannerRequest({ fields: value.fields, profilePathCatalog: value.profilePathCatalog });
}
