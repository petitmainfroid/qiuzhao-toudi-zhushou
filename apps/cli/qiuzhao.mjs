...
const commands = Object.freeze({
  ...
  vision: path.join(root, 'modules', 'page-vision', 'cli.mjs'),
  profile: path.join(root, 'modules', 'profile-page', 'cli.mjs'),
  autofill: path.join(root, 'modules', 'application-service', 'autofill-cli.mjs'),
  jobs: path.join(root, 'modules', 'job-repository', 'cli.mjs'),
  'job-agent': path.join(root, 'skills', 'job-hunting', 'cli.mjs'),
...
function usage() {
  return [
    ...
    '  vision <capture|status|cleanup|cleanup-expired>',
    '  profile <serve|status|stop>',
    '  autofill [--request-id <opaque-id>]  Fill unambiguous ordinary fields, then stop before submission',
    '  jobs <list|get|events|upsert|transition|score|protect|delete|restore>',
    '  job-agent serve  Start the separate Codex/Claude job-hunting MCP server',
