...
const plannerRequest = createAiPlannerRequest({
  fields: [
    ...
    {
      ref: "field:resume", section: "Attachments", label: "Resume", role: "textbox", required: true,
      hasValue: false, capability: "upload_saved_resume", safetyClass: "attachment", options: [], conditional: false
    }
  ],
  profilePathCatalog: [
...
const proposal = {
  ...
  decisions: [
    ...
    { kind: "upload_default_resume", ref: "field:resume" }
  ]
} as const;
...
describe("deterministic fail-closed policy compiler", () => {
  it("compiles only the exact no-work checkbox against the work collection", () => {
    const request = createAiPlannerRequest({
      fields: [{ ref: "field:no-work", section: "work.0", label: "没有工作经历", role: "checkbox", required: false,
        hasValue: true, capability: "set_boolean", safetyClass: "ordinary", options: [], conditional: false }],
      profilePathCatalog: [
        { path: "workExperiences", kind: "repeatable", hasValue: true, safetyClass: "ordinary" },
        { path: "projects", kind: "repeatable", hasValue: true, safetyClass: "ordinary" }
      ]
    });
    const input = { plannerRequest: request, proposal: { schemaVersion: 1, decisions: [{
      kind: "map_collection_empty", ref: "field:no-work", profilePath: "workExperiences"
    }] }, binding, authority, attemptBudget: 1 } as const;
    expect(compilePlan(input).decisions[0]).toEqual(expect.objectContaining({
      disposition: "set_boolean_from_collection_empty", profilePath: "workExperiences"
    }));
    expect(() => compilePlan({ ...input, proposal: { schemaVersion: 1, decisions: [{
      kind: "map_collection_empty", ref: "field:no-work", profilePath: "projects"
    }] } })).toThrow(expect.objectContaining({ code: "incompatible_decision" }));
  });

  it("compiles only a complete ordinary date range in left-to-right profile order", () => {
    const request = createAiPlannerRequest({
      fields: [{ ref: "field:range", section: "Education", label: "Dates", role: "textbox", required: true,
        hasValue: false, capability: "set_date_range", safetyClass: "ordinary", options: [], conditional: false }],
      profilePathCatalog: [
        { path: "education.0.startDate", kind: "date", hasValue: true, safetyClass: "ordinary" },
        { path: "education.0.endDate", kind: "date", hasValue: true, safetyClass: "ordinary" }
      ]
    });
    const result = compilePlan({ plannerRequest: request, proposal: { schemaVersion: 1, decisions: [{
      kind: "map_date_range", ref: "field:range", startProfilePath: "education.0.startDate", endProfilePath: "education.0.endDate"
    }] }, binding, authority, attemptBudget: 1 });
    expect(result.decisions[0]).toEqual(expect.objectContaining({
      disposition: "fill_date_range", startProfilePath: "education.0.startDate", endProfilePath: "education.0.endDate", maxAttempts: 1
    }));
    expect(() => compilePlan({ plannerRequest: request, proposal: { schemaVersion: 1, decisions: [{
      kind: "map_date_range", ref: "field:range", startProfilePath: "education.0.endDate", endProfilePath: "education.0.startDate"
    }] }, binding, authority, attemptBudget: 1 })).toThrow(expect.objectContaining({ code: "incompatible_decision" }));
  });
  it("compiles a start-only range only when the paired end is locally empty", () => {
    const request = createAiPlannerRequest({
      fields: [{ ref: "field:range-start", section: "work.0", label: "起止时间", role: "textbox", required: true,
        hasValue: false, capability: "set_date_range", safetyClass: "ordinary", options: [], conditional: false }],
      profilePathCatalog: [
    ...
    const input = { plannerRequest: request, proposal: { schemaVersion: 1, decisions: [{
      kind: "map_date_range_start", ref: "field:range-start", startProfilePath: "workExperiences.0.startDate"
    }] }, binding, authority, attemptBudget: 1 } as const;
    expect(compilePlan(input).decisions[0]).toEqual(expect.objectContaining({
      disposition: "fill_date_range_start", startProfilePath: "workExperiences.0.startDate"
    }));
  });
  it("binds every observed ref exactly once with an immutable plan id", () => {
    ...
    expect(plan.decisions.map(({ disposition, maxAttempts }) => ({ disposition, maxAttempts }))).toEqual([
      ...
      { disposition: "upload_default_resume", maxAttempts: 1 }
    ]);
  ...
  it("uploads only the single saved default PDF into one unambiguous empty resume field", () => {
    const replace = (replacement: unknown) => ({
      ...proposal,
      decisions: proposal.decisions.map((decision) => decision.ref === "field:resume" ? replacement : decision)
    });
    const missingResume = createAiPlannerRequest({
      fields: plannerRequest.fields,
      profilePathCatalog: plannerRequest.profilePathCatalog,
      defaultResume: { hasValue: false, mimeType: "application/pdf" }
    });
    expect(() => compile({ plannerRequest: missingResume }))
      .toThrow(expect.objectContaining({ code: "resume_value_state_mismatch" }));
    const occupiedResume = createAiPlannerRequest({
      fields: plannerRequest.fields.map((field) => field.ref === "field:resume" ? { ...field, hasValue: true } : field),
      profilePathCatalog: plannerRequest.profilePathCatalog,
      defaultResume: plannerRequest.defaultResume
    });
    expect(() => compile({ plannerRequest: occupiedResume }))
      .toThrow(expect.objectContaining({ code: "incompatible_decision" }));
    expect(() => compile({ proposal: replace({ kind: "map", ref: "field:resume", profilePath: "basics.fullName" }) }))
      .toThrow(expect.objectContaining({ code: "protected_action" }));
    const ambiguous = createAiPlannerRequest({
      fields: [...plannerRequest.fields, { ...plannerRequest.fields.at(-1)!, ref: "field:resume2" }],
      profilePathCatalog: plannerRequest.profilePathCatalog,
      defaultResume: plannerRequest.defaultResume
    });
    expect(() => compile({
      plannerRequest: ambiguous,
      proposal: {
        ...proposal,
        decisions: [...proposal.decisions, { kind: "manual", ref: "field:resume2", reason: "user_input_required" }]
      }
    })).toThrow(expect.objectContaining({ code: "incompatible_decision" }));
  });
