...
describe("privacy-safe semantic planner contract", () => {
  ...
  it("builds a closed AI-first request without scalar profile values", () => {
    ...
    expect(request.defaultResume).toEqual({ hasValue: false, mimeType: "application/pdf" });
  ...
  it("accepts only the closed mutually exclusive typed decision shapes", () => {
    const decisions = [
      ...
      { kind: "upload_default_resume", ref: "f7" }
      , { kind: "map_date_range", ref: "f8", startProfilePath: "education.0.startDate", endProfilePath: "education.0.endDate" }
      , { kind: "map_collection_empty", ref: "f9", profilePath: "workExperiences" }
      , { kind: "map_date_range_start", ref: "f10", startProfilePath: "workExperiences.0.startDate" }
    ];
    expect(parseAiDecisionProposal({ schemaVersion: 1, decisions }).decisions).toEqual(decisions);
  });

  it("exposes only Boolean default-resume availability and never a path or filename", () => {
    const request = createAiPlannerRequest({
      ...input(),
      defaultResume: { hasValue: true, mimeType: "application/pdf" }
    });
    expect(request.defaultResume).toEqual({ hasValue: true, mimeType: "application/pdf" });
    expect(Object.keys(request.defaultResume).sort()).toEqual(["hasValue", "mimeType"]);
    expect(JSON.stringify(request.defaultResume)).not.toMatch(/path|name|bytes|sha256/i);
  });
  ...
  it("publishes JSON Schemas whose objects are closed", () => {
    ...
    expect(validateProposal({
      schemaVersion: 1,
      decisions: [{ kind: "upload_default_resume", ref: "f1" }]
    })).toBe(true);
    expect(validateProposal({
      schemaVersion: 1,
