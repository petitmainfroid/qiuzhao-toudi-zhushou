...
import type { BinaryAtRestProtector } from "./types";
import { ProfileServiceError } from "./errors";

const DEFAULT_MAX_BYTES = 4 * 1024 * 1024;
const ABSOLUTE_MAX_PLAINTEXT_BYTES = 10 * 1024 * 1024;
const ABSOLUTE_MAX_CIPHERTEXT_BYTES = 12 * 1024 * 1024;
...
try {
  ...
  if ($requestText.Length -gt 18000000) { exit 40 }
  ...
  if ($request.op -eq 'protect') {
    if ($inputBytes.Length -gt 10485760) { exit 42 }
    ...
    if ($inputBytes.Length -gt 12582912) { exit 42 }
...
export class WindowsDpapiProtector implements BinaryAtRestProtector {
  ...
  constructor(options: WindowsDpapiProtectorOptions = {}) {
    ...
    if (!Number.isSafeInteger(this.timeoutMs) || this.timeoutMs < 100 || this.timeoutMs > 60_000
      || !Number.isSafeInteger(this.maxPlaintextBytes) || this.maxPlaintextBytes < 1 || this.maxPlaintextBytes > ABSOLUTE_MAX_PLAINTEXT_BYTES
      || !Number.isSafeInteger(this.maxCiphertextBytes) || this.maxCiphertextBytes < 1 || this.maxCiphertextBytes > ABSOLUTE_MAX_CIPHERTEXT_BYTES) fail();
  ...
  async protectBytes(plaintext: Uint8Array): Promise<string> {
    return this.invoke("protect", Buffer.from(plaintext.buffer, plaintext.byteOffset, plaintext.byteLength));
  }

  async unprotectBytes(protectedPayload: string): Promise<Uint8Array> {
    const bytes = decodeStrictBase64(protectedPayload, this.maxCiphertextBytes);
    const plaintextBase64 = await this.invoke("unprotect", bytes);
    return new Uint8Array(decodeStrictBase64(plaintextBase64, this.maxPlaintextBytes));
  }
