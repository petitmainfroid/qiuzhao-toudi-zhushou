// Record-level additive migrations remain compatible with the v4 encrypted
// envelope; the repository schema version changes only for envelope changes.
...
export type ExperienceType = "internship" | "work";
...
function normalizedExperienceType(source: Record<string, unknown>): ExperienceType {
  if (source.experienceType === "internship" || source.experienceType === "work") return source.experienceType;
  // Legacy records had one shared collection. Classify only from the local
  // record's own role/description so an imported internship remains routable
  // to internship-only ATS sections without exposing that content to agents.
  const signal = `${asString(source.role)} ${asString(source.description)}`;
  return /(?:\u5b9e\u4e60|intern)/iu.test(signal) ? "internship" : "work";
}
...
function normalizeWork(value: unknown): WorkExperienceRecord {
  ...
  return {
    ...
    experienceType: normalizedExperienceType(source),
...
function hasMeaningfulFields(record: object): boolean {
  return Object.entries(record).some(
    ([key, value]) => key !== "id" && key !== "experienceType" && typeof value === "string" && hasText(value)
  );
}
