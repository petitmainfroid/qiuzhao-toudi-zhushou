...
describe("privacy-safe page state", () => {
  ...
  it("excludes unproven or cross-origin frames and classifies destructive, consent, and default submit controls", () => {
    ...
    const root: CdpDomNode = {
      ...
      children: [element("html", {}, [element("body", {}, [
        element("form", {}, [
          ...
          element("button", { type: "button", class: "apply-form-array-card-delete__hash" }, [text("x".repeat(120))]),
    ...
    expect(state.controls.filter((control) => control.safety === "destructive")).toHaveLength(2);
  ...
  it("uses form-item labels, collapses date roots, and excludes internal or hidden fields", () => {
    backendNodeId = 1;
    const root: CdpDomNode = {
      backendNodeId: backendNodeId++, nodeType: 9, nodeName: "#document", frameId: "main-frame",
      children: [element("html", {}, [element("body", {}, [
    ...
    const state = buildPrivacySafePageState(root, session, new OpaqueReferenceRegistry(() => "logicalnonce"));
    expect(state.controls).toHaveLength(2);
    expect(state.controls[0]).toEqual(expect.objectContaining({
      tag: "custom", role: "textbox", required: true,
      semantics: expect.objectContaining({ label: "璧锋鏃堕棿" })
    }));
    expect(state.controls[1]).toEqual(expect.objectContaining({
      role: "combobox", semantics: expect.objectContaining({ label: "瀛﹀巻" })
    }));
  });

  it("recognizes a structural select input and an inert repeatable add link", () => {
    backendNodeId = 1;
    const root: CdpDomNode = {
      backendNodeId: backendNodeId++, nodeType: 9, nodeName: "#document", frameId: "main-frame",
      children: [element("html", {}, [element("body", {}, [element("section", {}, [
        element("h2", {}, [text("教育背景")]),
        element("a", { href: "javascript:void(0)" }, [text("添加")]),
        element("div", { class: "apply-field Select" }, [
          element("div", { class: "field-label" }, [text("学历")]),
          element("div", { class: "Dropdown-container" }, [
            element("label", { class: "Select-container select" }, [
              element("input", { type: "text", placeholder: "请选择" })
            ])
          ])
        ])
      ])])])]
    ...
    const state = buildPrivacySafePageState(root, session, new OpaqueReferenceRegistry(() => "mokanonse"));
    expect(state.controls).toEqual(expect.arrayContaining([
      expect.objectContaining({
        tag: "a", role: "button",
        semantics: expect.objectContaining({ label: "添加教育经历", name: "education_list.add" })
      }),
      expect.objectContaining({
        tag: "input", role: "combobox",
        semantics: expect.objectContaining({ label: "学历", placeholder: "请选择" })
      })
    ]));
  });

  it("indexes repeated form-array cards even when a section checkbox precedes the first field", () => {
    backendNodeId = 1;
    const card = () => element("div", { class: "apply-form-array-card__hash" }, [
      element("div", { class: "apply-form-array-card-content__hash" }, [
        element("div", { class: "form-item" }, [
          element("div", { class: "form-item-label" }, [text("公司名称")]),
    ...
    const root: CdpDomNode = {
      backendNodeId: backendNodeId++, nodeType: 9, nodeName: "#document", frameId: "main-frame",
      children: [element("html", {}, [element("body", {}, [element("section", {}, [
        element("h2", {}, [text("工作经历")]),
        element("input", { type: "checkbox", "aria-label": "没有工作经历" }),
        card(), card()
      ])])])]
    };
    const state = buildPrivacySafePageState(root, session, new OpaqueReferenceRegistry(() => "cardnonce"));
    const companies = state.controls.filter((control) => control.semantics.label === "公司名称");
    expect(companies.map((control) => control.sectionContext)).toEqual([
      { kind: "work", heading: "工作经历", recordIndex: 0 },
      { kind: "work", heading: "工作经历", recordIndex: 1 }
    ]);
  });

  it("groups a labelled year-month range without exposing its child controls", () => {
    backendNodeId = 1;
    const root: CdpDomNode = {
      backendNodeId: backendNodeId++, nodeType: 9, nodeName: "#document", frameId: "main-frame",
      children: [element("html", {}, [element("body", {}, [element("div", { class: "date_info" }, [
        element("div", { class: "title-field" }, [text("起止时间")]),
        element("input", { type: "text" }), element("input", { type: "text" }),
        element("input", { type: "text" }), element("input", { type: "text" }),
        element("input", { type: "checkbox" })
      ])])])]
    };
    const state = buildPrivacySafePageState(root, session, new OpaqueReferenceRegistry(() => "yearmonthnonce"));
    expect(state.controls).toHaveLength(2);
    expect(state.controls[0]).toEqual(expect.objectContaining({
      tag: "custom", inputType: "year-month-range", semantics: expect.objectContaining({ label: "起止时间" })
    }));
    expect(state.controls[1]).toEqual(expect.objectContaining({ role: "checkbox" }));
  });

  it("treats a two-sided period-month picker with one hidden input as a range", () => {
    backendNodeId = 1;
    const root: CdpDomNode = {
      backendNodeId: backendNodeId++, nodeType: 9, nodeName: "#document", frameId: "main-frame",
      children: [element("html", {}, [element("body", {}, [
        element("div", { class: "form-item" }, [
          element("div", { class: "form-item-label" }, [text("起止时间")]),
          element("div", { class: "atsx-date-picker atsx-date-picker-period-month", "data-cy": "internship[0].periodInput" }, [
            element("div", { class: "atsx-date-picker-period-month-label", "data-cy": "internship[0].periodInputBegin" }, [
              element("span", { "data-cy": "year" }, [text("YYYY")]),
              element("span", { "data-cy": "month" }, [text("MM")])
            ]),
            element("div", { class: "atsx-date-picker-period-month-label", "data-cy": "internship[0].periodInputEnd" }, [
              element("span", { "data-cy": "year" }, [text("YYYY")]),
              element("span", { "data-cy": "month" }, [text("MM")])
            ]),
            element("input", { type: "text", class: "atsx-date-picker-period-hidden-input" })
          ])
        ])
      ])])]
    };
    const state = buildPrivacySafePageState(root, session, new OpaqueReferenceRegistry(() => "twomonthnonce"));
    expect(state.controls).toHaveLength(1);
    expect(state.controls[0]).toEqual(expect.objectContaining({
      tag: "custom", inputType: "year-month-range", semantics: expect.objectContaining({ label: "起止时间" })
    }));
  ...
  it("prefers a structurally associated visual label over a generic custom-control label", () => {
    backendNodeId = 1;
    const root: CdpDomNode = {
      backendNodeId: backendNodeId++, nodeType: 9, nodeName: "#document", frameId: "main-frame",
      children: [element("html", {}, [element("body", {}, [element("section", {}, [
        element("h2", {}, [text("工作经历")]),
        element("div", { class: "custom-field" }, [
          element("div", { class: "visual-title" }, [text("性别")]),
          element("div", { "data-form-field-i18n-name": "请选择" }, [element("input")])
        ]),
        element("div", { class: "custom-field" }, [
          element("div", { class: "visual-title" }, [text("工作职责")]),
          element("div", { "data-form-field-i18n-name": "内容" }, [element("textarea")])
        ]),
        element("div", {}, [element("input", { placeholder: "请输入公司名称" })])
      ])])])]
    };
    const state = buildPrivacySafePageState(root, session, new OpaqueReferenceRegistry(() => "contextnonce"));
    expect(state.controls.map((control) => control.semantics.label)).toEqual(["性别", "工作职责", "请输入公司名称"]);
    expect(state.controls.every((control) => control.semantics.name === "work_experience_list")).toBe(true);
  });

  it("inventories hidden file inputs structurally without exposing a local filename", () => {
    backendNodeId = 1;
    const root: CdpDomNode = {
      backendNodeId: backendNodeId++, nodeType: 9, nodeName: "#document", frameId: "main-frame",
      children: [element("html", {}, [element("body", {}, [
        element("label", {}, [
          text("上传简历"),
          element("input", { type: "file", name: "resume_file", style: "display:none", value: "C:\\fakepath\\private.pdf" })
        ]),
        element("label", {}, [
          text("上传身份证"),
          element("input", { type: "file", name: "identity_file", class: "hidden-field" })
        ]),
        element("div", { class: "hidden-field" }, [
          element("input", { type: "text", "aria-label": "内部文本" })
        ])
      ])])]
    };
    ...
    expect(state.controls).toHaveLength(2);
    expect(state.controls.map((control) => control.safety)).toEqual(["file", "identity"]);
    expect(state.controls.map((control) => control.inputType)).toEqual(["file", "file"]);
    expect(JSON.stringify(state)).not.toContain("private.pdf");
    expect(JSON.stringify(state)).not.toContain("fakepath");
    expect(state.controls.some((control) => control.semantics.label === "内部文本")).toBe(false);
  });
