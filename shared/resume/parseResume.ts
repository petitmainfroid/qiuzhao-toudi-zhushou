...
export function mergeResumeIntoProfile(existing: CandidateProfile, parsed: CandidateProfile): ResumeMergeResult {
  ...
  // Resume parsing may detect identity data so the user can review it, but it
  // must never silently flow into a persisted profile. These fields require
  // deliberate entry/confirmation in the profile editor.
  const parsedBasic = structuredClone(parsed.basic) as unknown as Record<string, string>;
  parsedBasic.identityDocumentType = "";
  parsedBasic.identityDocumentNumber = "";
  mergeScalarObject(
