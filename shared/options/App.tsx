...
export function ProfileEditor({
  ...
  function updateWork<K extends keyof WorkExperienceRecord>(index: number, key: K, value: WorkExperienceRecord[K]) {
    update((draft) => {
      ...
      <div className="editor-layout">
        ...
        <div className="form-sections">
          ...
          <section className="form-card" id="work">
            <SectionHeader index="04" icon={<BriefcaseBusiness size={21} />} title="实习 / 工作经历" description="为每段真实经历标记类型，便于招聘页面按经历区块匹配；空白记录不会计入档案完成度。" />
            {profile.workExperiences.length === 0 ? <p className="empty-copy">尚未添加实习或工作经历。</p> : null}
            {profile.workExperiences.map((record, index) => (
              <article className="repeat-card" key={record.id}>
                <div className="repeat-heading">
                  <h3>{record.experienceType === "internship" ? "实习经历" : "工作经历"} {index + 1}</h3>
                  <button className="icon-text-button danger-button" type="button" aria-label={`删除经历 ${index + 1}`} onClick={() => update((draft) => { draft.workExperiences.splice(index, 1); })}><Trash2 size={16} />删除</button>
                </div>
                <div className="field-grid">
                  <label className="field"><span>经历类型</span><select aria-label="经历类型" value={record.experienceType} onChange={(event) => updateWork(index, "experienceType", event.target.value === "internship" ? "internship" : "work")}><option value="internship">实习</option><option value="work">工作</option></select></label>
            ...
            <button className="add-button" type="button" onClick={() => update((draft) => { draft.workExperiences.push(createWorkExperienceRecord()); })}><Plus size={17} />添加一段工作经历</button>
          </section>
